...
public class Player_Collider : MonoBehaviour
{
    ...
    private void Start()
    {
        walkParticle = transform.GetChild(3).GetComponent<ParticleSystem>();
        Level = 1;
        exp = 0f;
    }
    ...
    private void Update()
    {
        if (GiftMenuOpen) walkParticle.Stop();
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "h_menu_area")
        {
            ...
            hg_handler = curr_house.transform.GetChild(0).GetComponent<House_gift_handler>();
